...
func get_latest_devices(db *sql.DB) ([]device_status, error) {
        query := `
        SELECT hostname, logged_user, cpu_percent, ram_percent, disk_percent, uptime_seconds, MAX(checked_in_at) AS last_seen
        FROM checkins
        GROUP BY hostname
        ORDER BY hostname;`
        ...
        for rows.Next() {
                ...
                if err := rows.Scan(&d.Hostname, &d.LoggedUser, &d.CpuPercent, &d.RamPercent, &d.DiskPercent, &d.UptimeSeconds, &last_seen_str); err != nil {
