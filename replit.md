...
CoolRMM-Go (eventually just "CoolRMM") is a lightweight, self-built RMM tool replacing Pulseway/Kaseya at work. Written in Go. Designed to monitor ~180 Windows 11 endpoints across three groups: CorpA, CorpB, and CorpC (internal).
...
Replit's sandbox blocks all .git/config modifications — remotes cannot be added programmatically. Sync is manual: copy changed files from Replit to local machine, then commit and push via SSH from there. If you edit replit.md locally and push, paste the relevant section into chat and it will be updated here.
User Preferences
...
Agent Deployment (per machine)
    ...
    Copy coolrmm.conf.example → coolrmm.conf in the same folder as coolrmm-agent.exe
    Edit coolrmm.conf — set server_url to the real server IP and port
    Run coolrmm-agent.exe
