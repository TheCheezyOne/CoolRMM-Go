...
func Test_parse_config_valid(t *testing.T) {
        ...
        if cfg.ApiKey != "secret123" {
                t.Errorf("expected api_key 'secret123', got '%s'", cfg.ApiKey)
        }
}

// Test_parse_config_missing_key confirms an error when required keys are absent.
func Test_parse_config_missing_key(t *testing.T) {
        ...
        if err == nil {
                t.Fatal("parse_config_file() expected error for missing keys, got nil")
        }
}

// Test_parse_config_missing_api_key confirms an error when api_key is absent.
func Test_parse_config_missing_api_key(t *testing.T) {
        conf := write_temp_conf(t, "server_url=http://192.168.1.100:8080\n")
        defer os.Remove(conf)

        _, err := parse_config_file(conf)
        if err == nil {
                t.Fatal("parse_config_file() expected error for missing api_key, got nil")
        }
}
